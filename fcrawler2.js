...
// 🔹 Crawl Wikipedia images
async function crawlWikipediaImages(query) {
  try {
    const url = `https://en.wikipedia.org/wiki/${encodeURIComponent(query)}`;
    const res = await axios.get(url, { headers: { "User-Agent": "FwebFcards/1.0 (+https://fweb.africa)" }, timeout: 7000 });
    const $ = cheerio.load(res.data);
    const images = [];
    $("img").each((i, el) => {
      const src = $(el).attr("src");
      if (src && !src.startsWith("//upload.wikimedia.org/wikipedia/commons/thumb/")) return; // skip thumbnails
      images.push({ src: src.startsWith("http") ? src : `https:${src}`, title: $(el).attr("alt") || "Image" });
    });
    return images.slice(0, 15); // limit to 15
  } catch (err) {
    console.error("❌ Wikipedia images fetch failed:", err.message);
    return [];
  }
}

// 🔹 Crawl YouTube video links (just search results, no download)
async function crawlYouTubeVideos(query) {
  try {
    ...
    $("a").each((i, el) => {
      ...
      if (href && href.startsWith("/watch")) {
        videos.push({ url: `https://www.youtube.com${href}`, title: $(el).attr("title") || "YouTube Video" });
      }
    });
    return videos.slice(0, 10); // limit to 10
  } catch (err) {
    console.error("❌ YouTube videos fetch failed:", err.message);
    return [];
  }
}

// 🔹 Normal search handler (extended)
export async function handleNormalSearch(query) {
  ...
  const requests = selectedSources.map(async (buildUrl) => {
    ...
    try {
      const response = await axios.get(url, { headers: { "User-Agent": "FwebFcards/1.0 (+https://fweb.africa)" }, timeout: 8000 });
      const $ = cheerio.load(response.data);
      const title = $("title").first().text().trim() || url;
  ...
  let cards = results.filter((r) => r.status === "fulfilled" && r.value).map((r) => r.value);
  ...
  // 🔹 Crawl images and videos
  const [images, videos] = await Promise.all([crawlWikipediaImages(query), crawlYouTubeVideos(query)]);

  return {
    fcards: cards.length > 0 ? cards : [{ title: "No Results", url: null, snippet: "No fcards could be generated.", type: "fcards-empty" }],
    images,
    videos
  };
}
